...
import {
  useManagerBuildings,
  type ManagerBuildingSummary,
} from '../../hooks/useManagerBuildings'
import {
  managerBuildingsApi,
  type BuildingPayload,
  type FloorPayload,
  type FloorUpdatePayload,
} from '../../services/managerBuildingsApi'
...
export function AdminBuildingsPage() {
  ...
  const [activeBuilding, setActiveBuilding] =
    useState<ManagerBuildingSummary | null>(null)
  const [floorModal, setFloorModal] = useState(false)
  const [floorMode, setFloorMode] = useState<'create' | 'edit'>('create')
  const [activeFloor, setActiveFloor] = useState<
    ManagerBuildingSummary['floors'][number] | null
  >(null)
  ...
  const totals = filtered.reduce(
    (sum, item) => ({
      ...
      floors: sum.floors + item.floorCount,
      slots: sum.slots + item.totalSlots,
    }),
  ...
  async function saveFloor(payload: FloorPayload, floorId?: string) {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Không thể lưu tầng.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <AdminPageShell
      ...
      actions={
        <Card className="w-full lg:min-w-[40rem]">
    ...
    >
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <AdminStatCard
          label="Tòa nhà"
          value={isLoading ? '-' : totals.buildings}
          detail="Theo bộ lọc hiện tại"
          tone="sky"
        />
        <AdminStatCard
          label="Tầng"
          value={isLoading ? '-' : totals.floors}
          detail="Tổng số tầng"
          tone="violet"
        />
        <AdminStatCard
          label="Chỗ đỗ"
          value={isLoading ? '-' : totals.slots}
          detail="Tổng sức chứa"
          tone="emerald"
        />
      </div>
      {error && (
