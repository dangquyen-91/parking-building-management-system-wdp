...
export function AdminSlotsPage() {
  ...
  return <div className="pb-10">
    ...
    <div className="mx-auto max-w-[118rem] px-4 md:px-8 lg:px-10"><div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><AdminStatCard label="Ô đỗ ô tô" value={filters.stats.totalSlots} detail={`${filters.stats.occupiedSlots} đang dùng`} tone="violet" /><AdminStatCard label="Hàng xe máy" value={filters.stats.totalRows} detail="Tổng số hàng" tone="sky" /><AdminStatCard label="Sức chứa xe máy" value={filters.stats.rowCapacity} detail={`${filters.stats.rowOccupied} đang dùng`} tone="emerald" /><AdminStatCard label="Bảo trì" value={filters.stats.maintenanceSlots} detail="Ô đỗ ô tô" tone="amber" /></div>{error && <div className="mb-4 rounded-2xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-300">{error}</div>}<AdminParkingSpaceList isLoading={isLoading} hasError={Boolean(error)} filteredSlots={filters.filteredSlots} filteredRows={filters.filteredRows} visibleSlotFloors={filters.visibleSlotFloors} visibleRowFloors={filters.visibleRowFloors} slotsByFloor={filters.slotsByFloor} rowsByFloor={filters.rowsByFloor} buildingMap={filters.buildingMap} onEditSlot={(slot) => { setSlotMode('edit'); setActiveSlot(slot); setSubmitError(null); setSlotOpen(true) }} onDeleteSlot={(slot) => void deleteSlot(slot)} onEditRow={(row) => { setRowMode('edit'); setActiveRow(row); setSubmitError(null); setRowOpen(true) }} /></div>
