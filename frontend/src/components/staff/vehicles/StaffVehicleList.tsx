...
import {
  formatStaffVehicleDateTime,
  formatStaffVehicleDuration,
} from '../../../utils/staffVehicleUi'
...
export function StaffVehicleList({
    ...
    return (
      <Card className="border-dashed">
        <CardHeader className="items-center text-center">
          ...
          <CardDescription>
            Thử thay đổi biển số hoặc bộ lọc đang chọn.
          </CardDescription>
        </CardHeader>
      </Card>
    )
  ...
  return (
    <StaffTableShell
      eyebrow="Theo dõi trong bãi"
      title="Xe đang gửi"
      countLabel={`${sessions.length} xe`}
      minWidth="980px"
      columns={[
        { label: 'Biển số', className: 'w-[18%]' },
        { label: 'Phân loại', className: 'w-[19%]' },
        { label: 'Vị trí hiện tại', className: 'w-[27%]' },
        { label: 'Thời gian gửi', className: 'w-[22%]' },
        { label: 'Thao tác', className: 'w-[14%] text-right' },
      ]}
    >
      {sessions.map((session) => (
        <TableRow key={session._id}>
          <TableCell className="px-4 py-4 text-lg font-black tracking-[0.06em] text-foreground">
          ...
          <TableCell className="px-4 py-4">
            <p
              ...
              title={formatSessionSpot(session, floorMap)}
            >
              {formatSessionSpot(session, floorMap)}
            </p>
          </TableCell>
          <TableCell className="px-4 py-4">
            <p className="font-medium text-foreground">
              {formatStaffVehicleDateTime(session.entryTime)}
            </p>
          ...
          <TableCell className="px-4 py-4 text-right">
            <Button type="button" size="sm" onClick={() => onCheckout(session)}>
              Xử lý xe ra
