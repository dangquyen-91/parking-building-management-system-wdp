  ...
  return (
    <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {stats.map((stat, index) => (
        <div
          key={stat.label}
          className={`liquid-glass-card relative overflow-hidden rounded-xl bg-gradient-to-br ${tones[index]?.glow ?? tones[0].glow} via-transparent to-transparent p-4`}
        >
          <span className={`absolute inset-y-0 left-0 w-1 ${tones[index]?.bar ?? tones[0].bar}`} />
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-subtle">{stat.label}</p>
          <p className="mt-2 text-3xl font-black tracking-tight text-fg">
            {isLoading ? '-' : stat.value}
            <span className="ml-1 text-xs font-medium text-muted">xe</span>
          </p>
        </div>
      ))}
    </div>
