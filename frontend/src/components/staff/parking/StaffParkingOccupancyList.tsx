import { Bike, Building2, CarFront, ChevronDown, Layers3, Search } from 'lucide-react'
...
const CAPACITY_UI: Record<CapacityTone, {
  panel: string
  icon: string
  badge: string
  text: string
  ...
  available: {
    ...
    badge: 'border-emerald-300/70 bg-emerald-100 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-200',
  ...
  busy: {
    ...
    badge: 'border-sky-300/70 bg-sky-100 text-sky-800 dark:border-sky-800 dark:bg-sky-950/60 dark:text-sky-200',
    text: 'text-sky-700 dark:text-sky-300',
    progress: '[&_[data-slot=progress-indicator]]:bg-sky-500',
    dot: 'bg-sky-500',
  },
  warning: {
    panel: 'border-amber-300/90 bg-amber-50/35 dark:border-amber-800 dark:bg-amber-950/15',
    icon: 'bg-amber-500/15 text-amber-800 dark:text-amber-300',
    badge: 'border-amber-300 bg-amber-100 text-amber-900 dark:border-amber-800 dark:bg-amber-950/60 dark:text-amber-200',
    text: 'text-amber-800 dark:text-amber-300',
    progress: '[&_[data-slot=progress-indicator]]:bg-amber-500',
    dot: 'bg-amber-500',
  },
  full: {
    panel: 'border-rose-300/90 bg-rose-50/40 dark:border-rose-800 dark:bg-rose-950/15',
    icon: 'bg-rose-500/15 text-rose-800 dark:text-rose-300',
    badge: 'border-rose-300 bg-rose-100 text-rose-900 dark:border-rose-800 dark:bg-rose-950/60 dark:text-rose-200',
  ...
  maintenance: {
    ...
    badge: 'border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200',
...
const SLOT_UI: Record<ResidentSlotStatus, { card: string; badge: string; dot: string; label: string }> = {
  empty: {
    card: 'border-t-4 border-emerald-500 bg-card',
    badge: 'border-emerald-300 bg-emerald-100 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-200',
    dot: 'bg-emerald-500',
    label: 'Trống',
  },
  occupied: {
    card: 'border-t-4 border-sky-500 bg-card',
    badge: 'border-sky-300 bg-sky-100 text-sky-800 dark:border-sky-800 dark:bg-sky-950/60 dark:text-sky-200',
    dot: 'bg-sky-500',
    label: 'Đang đỗ',
  },
  reserved: {
...
export function StaffParkingOccupancyList({ items, isLoading, error }: StaffParkingOccupancyListProps) {
  if (error) {
    return (
    ...
    return (
      <Card className="border-dashed">
        <CardHeader className="items-center text-center">
          ...
          <CardDescription>Khi tòa nhà, tầng và khu đã được tạo, dữ liệu sẽ hiện ở đây.</CardDescription>
        </CardHeader>
      </Card>
    )
  }
...
function groupByBuilding(items: StaffParkingOccupancyItem[]): BuildingGroup[] {
  ...
  items.forEach((item) => {
    ...
    if (!floor) {
      floor = { floorNumber: item.floorNumber, total: 0, occupied: 0, available: 0, sections: [] }
      ...
      <CardHeader className="relative overflow-hidden border-b bg-gradient-to-r from-sky-100/90 via-background to-emerald-100/70 p-5 dark:from-sky-950/40 dark:to-emerald-950/30 md:p-6">
        ...
        <div className="relative flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardDescription className="font-bold uppercase tracking-[0.16em]">Tòa nhà</CardDescription>
              <CardTitle className="mt-1 text-2xl md:text-3xl">{building.name}</CardTitle>
          ...
          <div className="grid grid-cols-3 gap-2 sm:min-w-[24rem]">
            ...
            <BuildingMetric label="Đang chiếm" value={building.occupied} tone="violet" />
            <BuildingMetric label="Còn trống" value={building.available} tone="emerald" />
          </div>
        </div>
        ...
        <div className="relative mt-5 rounded-xl border border-white/60 bg-background/65 p-3 shadow-sm backdrop-blur dark:border-white/10">
          <div className="mb-2 flex items-center justify-between gap-3 text-xs font-semibold">
            <span className="text-muted-foreground">Công suất toàn tòa nhà</span>
          ...
          <Progress value={percent} className={`h-2.5 bg-muted/80 ${ui.progress}`} />
        </div>
      </CardHeader>
...
function FloorOccupancyCard({ floor }: { floor: BuildingGroup['floors'][number] }) {
      ...
      <CardHeader className="border-b border-border/70 bg-background/80 p-4 md:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            <span className={`flex size-11 shrink-0 items-center justify-center rounded-xl ${ui.icon}`}>
              <Layers3 className="size-5" />
            </span>
            <div>
              <CardTitle className="text-xl">Tầng {floor.floorNumber}</CardTitle>
              <CardDescription className="mt-1">{floor.sections.length} khu đang được theo dõi</CardDescription>
            </div>
          </div>
        ...
        <Progress value={percent} className={`mt-4 h-2 bg-muted ${ui.progress}`} />
      </CardHeader>
...
function SectionCard({ item }: { item: StaffParkingOccupancyItem }) {
  ...
  return (
    <div className={`overflow-hidden rounded-2xl border shadow-sm transition-shadow hover:shadow-md ${ui.panel}`}>
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${ui.icon}`}>
              <VehicleIcon className="size-5" />
            </span>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h4 className="text-base font-bold text-foreground">Khu {getFloorSection(item.section)}</h4>
                <Badge variant="secondary">{formatVehicleType(item.vehicleType)}</Badge>
              </div>
              <p className="mt-1 text-xs font-medium text-muted-foreground">
                {item.floorType ? FLOOR_TYPE_LABELS[item.floorType] ?? item.floorType : 'Khu đỗ xe'}
              </p>
            </div>
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          ...
          <SectionMetric label="Còn trống" value={item.available} tone={ui.text} />
        </div>

        <div className="mt-4">
          <div className="mb-2 flex items-center justify-between text-xs font-semibold">
            <span className="text-muted-foreground">Mức sử dụng</span>
            <span className={ui.text}>{item.occupied}/{item.total} · {formatPercent(percent)}</span>
          </div>
        ...
        {hasDetails ? (
          ...
          >
            {item.vehicleType === 'car' ? 'Xem trạng thái ô đỗ' : 'Xem sức chứa từng hàng'}
  ...
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">Trạng thái ô đỗ</p>
        <div className="flex flex-wrap gap-1.5" aria-label="Chú thích trạng thái ô đỗ">
          {(Object.keys(SLOT_UI) as ResidentSlotStatus[]).map((status) => (
            <Badge key={status} variant="outline" className={SLOT_UI[status].badge}>
              <span className={`size-1.5 rounded-full ${SLOT_UI[status].dot}`} />
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">
        {slots.map((slot) => {
          ...
          return (
            <div key={slot.id} className={`rounded-xl border p-3 shadow-sm ${statusUi.card}`}>
              <div className="flex items-center justify-between gap-2">
                <span className="font-bold text-foreground">{slot.code}</span>
                <Badge variant="outline" className={statusUi.badge}>{statusUi.label}</Badge>
              </div>
...
function MotorcycleRowDetails({ rows }: { rows: StaffParkingOccupancyItem['rowDetails'] }) {
  return (
    <div>
      <p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">Sức chứa từng hàng xe máy</p>
      <div className="grid gap-2 sm:grid-cols-2 2xl:grid-cols-3">
        {rows.map((row) => {
          const available = Math.max(0, row.capacity - row.occupied)
          const percent = calculatePercent(row.occupied, row.capacity)
          const tone = row.status === 'maintenance' ? 'maintenance' : getCapacityTone(percent)
          const ui = CAPACITY_UI[tone]

          return (
            <div key={row.id} className={`rounded-xl border p-3 shadow-sm ${ui.panel}`}>
              <div className="flex items-center justify-between gap-2">
                <span className="font-bold text-foreground">Hàng {row.code}</span>
                <Badge variant="outline" className={ui.badge}>{formatCapacityStatus(tone, available)}</Badge>
              </div>
              <div className="mt-3 flex items-center justify-between text-xs font-medium">
                <span className="text-muted-foreground">Đang dùng {row.occupied}/{row.capacity}</span>
                <span className={ui.text}>{formatPercent(percent)}</span>
              </div>
              <Progress value={percent} className={`mt-2 h-2 bg-muted ${ui.progress}`} />
            </div>
          )
        })}
      </div>
...
function BuildingMetric({ label, value, tone }: { label: string; value: number; tone: 'sky' | 'violet' | 'emerald' }) {
  const tones = {
    sky: 'border-sky-200 bg-sky-50/90 text-sky-800 dark:border-sky-900 dark:bg-sky-950/50 dark:text-sky-200',
    violet: 'border-violet-200 bg-violet-50/90 text-violet-800 dark:border-violet-900 dark:bg-violet-950/50 dark:text-violet-200',
    emerald: 'border-emerald-200 bg-emerald-50/90 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/50 dark:text-emerald-200',
  }

  return (
    <div className={`rounded-xl border px-3 py-2 text-center shadow-sm ${tones[tone]}`}>
...
function MiniMetric({ label, value, emphasis = false }: { label: string; value: number; emphasis?: boolean }) {
  return (
    <div className="rounded-lg border border-border/80 bg-muted/30 px-3 py-1.5">
      ...
      <strong className={emphasis ? 'text-emerald-700 dark:text-emerald-300' : 'text-foreground'}>{value}</strong>
    </div>
  )
}

function SectionMetric({ label, value, tone = 'text-foreground' }: { label: string; value: number; tone?: string }) {
  return (
...
function calculatePercent(occupied: number, total: number) {
  return total > 0 ? Math.min(100, Math.round((occupied / total) * 1000) / 10) : 0
}
...
function getCapacityTone(percent: number): Exclude<CapacityTone, 'maintenance'> {
...
function formatCapacityStatus(tone: CapacityTone, available?: number) {
  ...
  if (tone === 'warning') return available === undefined ? 'Sắp đầy' : `Sắp đầy · còn ${available}`
  if (tone === 'busy') return available === undefined ? 'Đang đông' : `Đang đông · còn ${available}`
