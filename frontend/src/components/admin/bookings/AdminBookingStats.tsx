...
export function AdminBookingStats({
  ...
  const revenue = bookings
    .filter((item) => item.status === 'paid' || item.status === 'used')
  ...
  return (
    <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <AdminStatCard
        ...
        detail="Chưa hoàn tất giao dịch"
        tone="amber"
      />
      <AdminStatCard
        label="Sẵn sàng sử dụng"
        value={isLoading ? '-' : paid}
        detail={`${used} booking đã sử dụng`}
        tone="emerald"
      />
      <AdminStatCard
        label="Doanh thu booking"
        value={isLoading ? '-' : formatAdminCurrency(revenue)}
        detail="Đã thanh toán và sử dụng"
        tone="sky"
      />
    </div>
  )
