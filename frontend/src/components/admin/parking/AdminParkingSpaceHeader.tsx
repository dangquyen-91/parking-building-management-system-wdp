          ...
          <div className="flex flex-col gap-3 rounded-lg border border-theme bg-badge p-3 sm:flex-row sm:items-end">
            <div className="grid flex-1 gap-3 sm:grid-cols-2">
              <AdminField label="Tòa nhà">
                <select className={adminInputClass} value={buildingFilter} onChange={(event) => onBuildingFilterChange(event.target.value)}>
              ...
              <AdminField label="Tầng / Khu">
                <select className={adminInputClass} value={floorFilter} onChange={(event) => onFloorFilterChange(event.target.value)}>
                  <option value="all">Tất cả</option>
                  {floors.map((item) => (
                    <option key={item._id} value={item._id}>{formatFloorLabel(item)}</option>
                  ))}
                </select>
              </AdminField>
            </div>
