...
function SlotGroup({ group, slotsByFloor, onEdit, onDelete }: { group: FloorGroup; slotsByFloor: Map<string, ParkingSlot[]>; onEdit: (slot: ParkingSlot) => void; onDelete: (slot: ParkingSlot) => void }) {
  ...
  return <Card><CardHeader className="flex-row items-end justify-between"><div><p className="text-xs text-muted-foreground">{group.buildingName ?? 'Tòa nhà'} · Tầng {group.floorNumber}</p><CardTitle className="mt-1">Sơ đồ ô đỗ ô tô</CardTitle></div><Badge variant="secondary">{slots.length} ô</Badge></CardHeader><CardContent><FloorTabs floors={group.floors} value={floor?._id ?? ''} counts={slotsByFloor} onChange={setFloorId} /><div className="mt-4 max-h-[34rem] overflow-y-auto"><AdminParkingSlotDiagram slots={slots} onEdit={onEdit} onDelete={onDelete} /></div></CardContent></Card>
}
...
function RowGroup({ group, rowsByFloor, onEdit }: { group: FloorGroup; rowsByFloor: Map<string, ParkingRow[]>; onEdit: (row: ParkingRow) => void }) {
  ...
  return <Card><CardHeader className="flex-row items-end justify-between"><div><p className="text-xs text-muted-foreground">{group.buildingName ?? 'Tòa nhà'} · Tầng {group.floorNumber}</p><CardTitle className="mt-1">Hàng xe máy</CardTitle></div><Badge variant="secondary">{rows.length} hàng</Badge></CardHeader><CardContent><FloorTabs floors={group.floors} value={floor?._id ?? ''} counts={rowsByFloor} onChange={setFloorId} /><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{rows.map((row) => <Card key={row._id} className="shadow-none"><CardContent className="flex items-center justify-between p-3"><div><p className="font-medium">{row.rowCode}</p><p className="text-xs text-muted-foreground">{row.occupiedCount}/{row.capacity} đang dùng</p></div><Button size="sm" variant="outline" onClick={() => onEdit(row)}>Sửa</Button></CardContent></Card>)}</div></CardContent></Card>
}

function FloorTabs<T>({ floors, value, counts, onChange }: { floors: Floor[]; value: string; counts: Map<string, T[]>; onChange: (id: string) => void }) {
  return <Tabs value={value} onValueChange={onChange}><TabsList className="h-auto max-w-full justify-start overflow-x-auto">{floors.map((floor) => <TabsTrigger key={floor._id} value={floor._id}>Khu {getFloorSection(floor.section)} ({counts.get(floor._id)?.length ?? 0})</TabsTrigger>)}</TabsList></Tabs>
}
