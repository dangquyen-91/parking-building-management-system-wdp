...
export function AdminChartShell({ eyebrow, title, description, children, tone }: { eyebrow: string; title: string; description: string; children: ReactNode; tone: 'violet' | 'emerald' | 'amber' }) {
  void tone
  return <Card><CardHeader><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">{eyebrow}</p><CardTitle>{title}</CardTitle><CardDescription>{description}</CardDescription></CardHeader><CardContent><div className="h-72">{children}</div></CardContent></Card>
}
