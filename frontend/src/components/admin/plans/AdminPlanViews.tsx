...
export function AdminPlanFilters({
  ...
  vehicleFilter: AdminPlanVehicleFilter
  statusFilter: AdminPlanStatusFilter
  onVehicleFilterChange: (value: AdminPlanVehicleFilter) => void
  onStatusFilterChange: (value: AdminPlanStatusFilter) => void
}) {
  return (
    <Card className="w-full xl:min-w-[30rem]">
      <CardContent className="grid gap-3 p-4 sm:grid-cols-2">
        <OperationField label="Loại xe">
          <NativeSelect
            value={vehicleFilter}
            onChange={(e) =>
              onVehicleFilterChange(e.target.value as AdminPlanVehicleFilter)
            }
          >
        ...
        <OperationField label="Trạng thái">
          <NativeSelect
            value={statusFilter}
            onChange={(e) =>
              onStatusFilterChange(e.target.value as AdminPlanStatusFilter)
            }
          >
            <NativeSelectOption value="all">Tất cả</NativeSelectOption>
            ...
            <NativeSelectOption value="inactive">Đã tắt</NativeSelectOption>
          </NativeSelect>
        </OperationField>
      </CardContent>
    </Card>
...
export function AdminPlanStats({
  plans,
  isLoading,
}: {
  plans: ManagerPlan[]
  ...
  return (
    <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <AdminStatCard
        ...
        detail="Tất cả loại xe"
        tone="violet"
      />
      <AdminStatCard
        label="Đang hoạt động"
        value={isLoading ? '-' : plans.filter((item) => item.isActive).length}
        detail="Có thể đăng ký"
        tone="emerald"
      />
      <AdminStatCard
        label="Gói xe máy"
        value={
          isLoading
            ? '-'
            : plans.filter((item) => item.vehicleType === 'motorcycle').length
        }
        detail="Dùng sức chứa chung"
        tone="sky"
      />
      <AdminStatCard
        label="Gói ô tô"
        value={
          isLoading
            ? '-'
            : plans.filter((item) => item.vehicleType === 'car').length
        }
        detail="Chọn ô cố định"
        tone="amber"
      />
    </div>
  )
...
export function AdminPlanList({
  ...
  return (
    <AdminTableShell
      ...
      minWidth="1050px"
      columns={[
        { label: 'Gói', className: 'w-[26%]' },
        { label: 'Loại xe', className: 'w-[12%]' },
        { label: 'Thời hạn', className: 'w-[12%]' },
        { label: 'Giá', className: 'w-[15%]' },
        { label: 'Trạng thái', className: 'w-[14%]' },
        { label: 'Thao tác', className: 'w-[21%] text-right' },
      ]}
    >
      {plans.map((plan) => (
        <TableRow key={plan._id}>
          <TableCell className="px-4 py-4">
            ...
            >
              {plan.code} · {plan.description || 'Không có mô tả'}
            </p>
          </TableCell>
          <TableCell className="px-4 py-4">
            {plan.vehicleType === 'car' ? 'Ô tô' : 'Xe máy'}
          </TableCell>
          <TableCell className="px-4 py-4">{plan.durationDays} ngày</TableCell>
          <TableCell className="px-4 py-4 font-bold">
            {formatAdminCurrency(plan.price)}
          </TableCell>
          <TableCell className="px-4 py-4">
    ...
    <Dialog open={Boolean(plan)} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <form onSubmit={submit}>
          <DialogHeader>
            <DialogDescription>Admin // Gói gửi xe</DialogDescription>
            <DialogTitle>Chỉnh sửa gói</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-5">
            <OperationField label="Tên gói">
              <Input
                value={name}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <OperationField label="Thời hạn (ngày)">
                <Input
                  type="number"
                  min="1"
                  value={durationDays}
