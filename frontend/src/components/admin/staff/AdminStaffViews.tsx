...
function staffIdFromSession(session: GateSession) {
  return typeof session.staffId === 'string'
    ? session.staffId
    : session.staffId?._id
}
      ...
      <CardContent className="grid gap-3 p-4 sm:grid-cols-[1fr_12rem]">
        ...
        <Label className="grid gap-2 text-xs text-muted-foreground">
          ...
          <NativeSelect
            value={statusFilter}
            onChange={(e) =>
              onStatusFilterChange(e.target.value as AdminStaffStatusFilter)
            }
          >
...
export function AdminStaffStats({
  ...
  const handling = new Set(sessions.map(staffIdFromSession).filter(Boolean))
    .size
  return (
    <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <AdminStatCard
        ...
        detail="Tài khoản có vai trò staff"
      />
      <AdminStatCard
        label="Đang hoạt động"
        value={isLoading ? '-' : active}
        detail={`${staff.length - active} tài khoản đã khóa`}
        tone="emerald"
      />
      <AdminStatCard
        label="Đang phụ trách"
        value={isLoading ? '-' : handling}
        detail="Có phiên gửi xe hoạt động"
        tone="sky"
      />
      <AdminStatCard
        label="Xe đang theo dõi"
        value={isLoading ? '-' : sessions.length}
        detail="Tổng phiên chưa checkout"
        tone="amber"
      />
    </div>
  )
...
export function AdminStaffList({
  ...
  return (
    <AdminTableShell
      eyebrow="Đội ngũ vận hành"
      title="Danh sách nhân viên"
      countLabel={`${staff.length} nhân viên`}
      minWidth="980px"
      columns={[
        { label: 'Nhân viên', className: 'w-[27%]' },
        { label: 'Liên hệ', className: 'w-[23%]' },
        { label: 'Trạng thái', className: 'w-[18%]' },
        { label: 'Xe phụ trách', className: 'w-[14%]' },
        { label: 'Biển số theo dõi', className: 'w-[18%]' },
      ]}
    >
      {staff.map((user) => {
          ...
          <TableRow key={user._id}>
            ...
            <TableCell className="px-4 py-4">
              <p
                ...
                title={staffSessions
                  .map((session) => session.licensePlate)
                  .join(', ')}
              >
