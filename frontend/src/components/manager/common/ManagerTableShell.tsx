...
type ManagerTableColumn = { label: string; className?: string }

type ManagerTableShellProps = {
  eyebrow: string
  title: string
  countLabel: string
...
export function ManagerTableShell({
  eyebrow,
  title,
  countLabel,
  columns,
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-card text-card-foreground shadow-sm">
      ...
      <Table className="table-fixed" style={{ minWidth }}>
        <TableHeader className="bg-muted/55">
          <TableRow className="hover:bg-muted/55">
            {columns.map((column) => (
              <TableHead key={column.label} className={`h-12 px-4 text-xs font-semibold text-muted-foreground ${column.className ?? ''}`}>
                {column.label}
              </TableHead>
            ))}
          </TableRow>
      ...
      <div className="flex flex-col gap-3 border-t border-border px-4 py-3 sm:flex-row sm:items-center sm:justify-between md:px-5">
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          <span>
            Hiển thị {rows.length === 0 ? 0 : firstRow + 1}-{Math.min(firstRow + pageSize, rows.length)} trong {rows.length}
          </span>
          <Select
          ...
          >
            <SelectTrigger className="h-8 w-[92px] rounded-lg text-xs" aria-label="Số dòng mỗi trang">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
