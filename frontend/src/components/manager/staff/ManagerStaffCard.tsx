...
export function ManagerStaffCard({ staff, sessions }: ManagerStaffCardProps) {
  ...
  return (
    <article className="liquid-glass-card rounded-2xl border border-theme p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="mb-3 flex size-11 items-center justify-center rounded-2xl border border-theme bg-gradient-to-br from-sky-500/20 to-emerald-500/20 text-sm font-black text-fg">
            {(staff.fullName || staff.email).slice(0, 2).toUpperCase()}
          </div>
          <p className="truncate text-lg font-black text-fg">{staff.fullName}</p>
