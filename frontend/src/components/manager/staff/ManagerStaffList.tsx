...
export function ManagerStaffList({
  ...
  return (
    <ManagerTableShell
      ...
      minWidth="1080px"
      columns={[
        { label: 'Nhân viên', className: 'w-[20%]' },
        { label: 'Liên hệ', className: 'w-[16%]' },
        { label: 'Trạng thái', className: 'w-[13%]' },
        { label: 'Xe phụ trách', className: 'w-[11%]' },
        { label: 'Phân loại', className: 'w-[12%]' },
        { label: 'Biển số theo dõi', className: 'w-[16%]' },
      ...
      {staff.map((user) => {
        const staffSessions = sessions.filter(
          (session) =>
            (typeof session.staffId === 'string'
              ? session.staffId
              : session.staffId?._id) === user._id,
        )
        const cars = staffSessions.filter(
          (session) => session.vehicleType === 'car',
        ).length
        ...
        return (
          <TableRow key={user._id}>
            ...
            <TableCell className="px-4 py-4">
              <p className="text-foreground">Ô tô: {cars}</p>
