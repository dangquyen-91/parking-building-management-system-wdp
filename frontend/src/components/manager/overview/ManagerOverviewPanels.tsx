...
export function ManagerOverviewCapacity({ floors }: { floors: ManagerOccupancyFloor[] }) {
  return (
    <section className="liquid-glass-card rounded-2xl border border-theme p-4 shadow-sm md:p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] uppercase tracking-[0.18em] text-subtle">Công suất</p>
          <h2 className="mt-1 text-lg font-black text-fg">Tình trạng từng tầng</h2>
        </div>
      ...
      {floors.length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {floors.slice(0, 6).map((floor) => (
            <article key={floor.floorId} className="rounded-2xl border border-theme bg-badge/70 p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:bg-ghost hover:shadow-lg">
    ...
    <section className="liquid-glass-card rounded-2xl border border-theme p-4 shadow-sm md:p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-lg font-black text-fg">Xe vừa vào bãi</h2>
        </div>
      ...
      {sessions.length === 0 ? (
        ...
        <div className="space-y-2">
          {sessions.slice(0, 5).map((session) => (
            <article key={session._id} className="flex items-center justify-between gap-3 rounded-2xl border border-theme bg-badge/70 p-3 transition-all hover:-translate-y-0.5 hover:bg-ghost">
    ...
    <section className="liquid-glass-card rounded-2xl border border-theme p-4 shadow-sm md:p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-lg font-black text-fg">Đã thanh toán, chưa sử dụng</h2>
        </div>
      ...
      {bookings.length === 0 ? (
        ...
        <div className="space-y-2">
          {bookings.slice(0, 5).map((booking) => (
            <article key={booking._id} className="flex items-center justify-between gap-3 rounded-2xl border border-theme bg-badge/70 p-3 transition-all hover:-translate-y-0.5 hover:bg-ghost">
              <div className="min-w-0">
  ...
  return (
    <section className="liquid-glass-card rounded-2xl border border-theme p-4 shadow-sm md:p-5">
      <div className="mb-4">
        ...
        <h2 className="mt-1 text-lg font-black text-fg">Cảnh báo vận hành</h2>
      </div>
      ...
      {alerts.length === 0 ? (
        ...
        <div className="space-y-2">
          {alerts.slice(0, 6).map((alert) => (
            <Link key={alert.id} to={alert.to} className="block rounded-2xl border border-theme bg-badge/70 p-3 transition-all hover:-translate-y-0.5 hover:bg-ghost">
              <div className="flex items-start justify-between gap-3">
                <div>
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {QUICK_LINKS.map((item) => (
        <Link key={item.to} to={item.to} className="group relative overflow-hidden rounded-2xl border border-theme bg-badge p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:bg-ghost hover:shadow-lg">
          <span className="absolute right-4 top-4 h-8 w-8 rounded-full bg-sky-500/10 transition-transform group-hover:scale-125" />
