...
export function ManagerSubscriptionList({ subscriptions, activePlates, snapshotTime, loading }: { subscriptions: ManagerSubscription[]; activePlates: Set<string>; snapshotTime: number; loading: boolean }) {
  ...
  return (
    <ManagerTableShell eyebrow="Danh sách" title="Người đã đăng ký gói" countLabel={`${subscriptions.length} người dùng`} minWidth="1100px" columns={[
      { label: 'Biển số', className: 'w-[18%]' }, { label: 'Người mua', className: 'w-[21%]' }, { label: 'Gói đăng ký', className: 'w-[21%]' }, { label: 'Thời hạn', className: 'w-[24%]' }, { label: 'Trạng thái', className: 'w-[16%]' },
    ]}>
